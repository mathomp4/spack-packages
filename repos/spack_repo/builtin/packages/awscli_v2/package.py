...
class AwscliV2(PythonPackage):
    ...
    # Building the bundled completion index imports every AWS CLI command and
    # can hang in an isolated build environment.  The index is used only by
    # aws_completer; the aws command itself does not require it.
